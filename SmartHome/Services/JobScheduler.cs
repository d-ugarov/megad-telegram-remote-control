...
public class JobScheduler : JobSchedulerBase
{
    ...
    {
        var homeProcessor = scope.ServiceProvider.GetRequiredService<IHomeProcessor>();
        var updatedStatuses = await homeProcessor.UpdateHomeStateAsync();
        await homeProcessor.ProcessSchedulerRulesAsync(updatedStatuses);
    }
}